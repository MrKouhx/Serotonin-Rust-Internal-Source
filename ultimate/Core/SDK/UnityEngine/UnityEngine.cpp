...
#pragma optimize( "", on )
	...
	auto Camera::viewMatrixx() -> VMatrix
	{
		uintptr_t* camera_ptr = NULL;
		static auto fn = reinterpret_cast<uintptr_t * (*)()>(CIl2Cpp::il2cpp_resolve_icall(XS("UnityEngine.Camera::get_main()")));
		camera_ptr = fn();

		if (!camera_ptr)
			return VMatrix();

		uintptr_t camera = *reinterpret_cast<uintptr_t*>((uintptr_t)camera_ptr + 0x10);
		if (!camera)
			return VMatrix();

		VMatrix matrifx = *reinterpret_cast<VMatrix*>(camera + 0x30C);
		if (matrifx.is_empty())
			return VMatrix();
		...
		auto matrix = *reinterpret_cast<VMatrix*>(camera_ + 0x30c);
		if (!matrix.m) {
			return {};
		}
		return matrix;
