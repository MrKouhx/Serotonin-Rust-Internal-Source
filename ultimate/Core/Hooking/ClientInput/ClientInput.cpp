...
void Hooks::ClientInput(AssemblyCSharp::BasePlayer* a1, AssemblyCSharp::InputState* a2)
{
	...
	if (IsAddressValid(Features().BaseProjectile) && IsAddressValid(Features().LocalPlayer))
	{
		...
		{
				Features().BaseProjectile->stancePenalty() = 0.00f;

				Features().BaseProjectile->aimconePenalty() = 0.00f;

				Features().BaseProjectile->aimCone() = -5.00f;

				Features().BaseProjectile->hipAimCone() = -5.00f;
				Features().BaseProjectile->aimConePenaltyMax() = 0.00f;
				Features().BaseProjectile->stancePenaltyScale() = 0.00f;

				Features().BaseProjectile->aimconePenaltyPerShot() = 0.00f;
		}
		//force automatic
				...
				for (int i = 0; i < size; i++)
				{
					...
					if (m_settings::IgnoreArms)
					{
							...
							|| material->get_name()->Contains(XS("Arm"))) // full arms 
						{
							...
							auto ChamsColor = Color{ ColorSettings::armm_chamss.r * m_settings::glowvalue, ColorSettings::armm_chamss.g * m_settings::glowvalue, ColorSettings::armm_chamss.b * m_settings::glowvalue, ColorSettings::armm_chamss.a };
					...
					else if (material->get_name()->Contains(XS("Hand")) // hand
						|| material->get_name()->Contains(XS("Glove")) // globves
						|| material->get_name()->Contains(XS("Arm"))) // full arms 
					{
						continue;
					...
					auto ChamsColor = Color{ ColorSettings::weapon_chamss.r * m_settings::glowvalue, ColorSettings::weapon_chamss.g * m_settings::glowvalue, ColorSettings::weapon_chamss.b * m_settings::glowvalue, ColorSettings::weapon_chamss.a };
